...
impl SSTable {
    pub fn write(
        ...
        for (i, (key, value)) in data.into_iter().enumerate() {
            ...
            let should_share_values = policy != CompressionPolicy::Fastest;

            let (value_type, value_offset) = if should_share_values && value_store.contains_key(&value) {
